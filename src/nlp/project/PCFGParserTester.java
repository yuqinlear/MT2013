  ...
  static class BaselineParser implements Parser {
    ...
    public Tree<String> getBestParse(List<String> sentence) {
      ...
      if (knownParses.keySet().contains(tags)) {
        annotatedBestParse = getBestKnownParse(tags, sentence);
      } else {
    ...
    private Tree<String> getBestKnownParse(List<String> tags, List<String> words) {
      Tree<String> knownTree = knownParses.getCounter(tags).argMax();
      int wordIndex = 0;
      for (Tree<String> child : knownTree.getPreOrderTraversal()) {
    	 if (child.isLeaf()) child.setLabel(words.get(wordIndex++));
      }
      return knownTree;
    }
...
//      System.out.println(uc);
  ...
  static class Grammar {
    ...
    void addBinary(BinaryRule binaryRule) {
    ...
    void addUnary(UnaryRule unaryRule) {
...
//          if (path.size() == 2) continue;
    ...
    private static Map<UnaryRule, List<String>> computeUnaryClosure(Collection<UnaryRule> unaryRules) {
      ...
      for (UnaryRule unaryRule : pathCosts.keySet()) {
        ...
        if (path.size()<2 && unaryRule.child.equals(unaryRule.parent))continue;
...
//    String testMode = "test";
    String testMode = "in-domain";
    int maxTrainLength = 20;
    int maxTestLength = 20;
    String testFileName = "en-web-weblogs-test.sentences";
//    String testFileName = "train50k_len50.2.mrg";
    ...
    if (argMap.containsKey("-file")) {
    	testFileName = argMap.get("-file");
        System.out.println("file: " + basePath + testFileName);
    }
    if (argMap.containsKey("-test")) {
      testMode = "test";
      System.out.println("Testing on final test data.");
    } else {
      System.out.println("Testing on dev data.");
    }
...
//      List<Tree<String>> testTreesOutOfDomain = readTrees(basePath, 3, 3, maxTestLength);
//      System.out.println("done. (" + testTreesOutOfDomain.size() + " trees)");
...
//      startTime = endTime;      
//      System.out.println("Testing out-of-domain ... ");
...
//    }
  ...
  private static void labelTestSet(String path, String filename, Parser parser, boolean verbose) throws Exception {
    BufferedReader reader = new BufferedReader(new FileReader(path + filename));
		BufferedWriter writer = new BufferedWriter(new FileWriter(path + filename +"_output.txt"));
		String line = "";
		int notPossibleParse = 0;
		long startTime = 0, endTime = 0;
    while ((line = reader.readLine()) != null) {
      List<String> testSentence = Arrays.asList(line.split(" "));
      startTime = System.currentTimeMillis();
      ...
      writer.write(guessedTree.toString() + "\n"); 
      writer.flush();
      if (verbose) {
    ...
    System.out.println("not possbile to be parsed out: " + notPossibleParse);
    writer.close();
  }
...
//      System.out.println(Trees.PennTreeRenderer.render(normalizedTree));
    ...
    System.out.println("the number of trees below maxlength: " + actualTestTreeNum);
